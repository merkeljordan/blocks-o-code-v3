...
First, functionally: the system has to detect which blocks are connected and in what order, validate that sequence against our grammar rules, and then execute on command—driving LEDs, audio, and the display—with real‑time feedback in the companion app.  
...
“All 15 blocks share a common hardware platform. Each one has an ESP32 microcontroller, a 2.8‑inch TFT touch display, a 4×4 LED matrix, addressable LEDs, and an audio path with an amplifier and speaker, all powered over USB‑C.  
...
**▶ PLAY: Demo — Scenario 1 (Brain → LED Color Flash) — `demo-scenario1-led-flash.mp4` (~0:45)**  
**Jordan (over video)**

“Here we connect the Brain and an LED Color Flash block. The Brain scans the I2C bus, sends the new configuration to the app, and you can see the app update to show a valid sequence.  
...
**Jordan (before Scenario 2)**

“The second scenario is Brain plus a Music Sequence block.”

**▶ PLAY: Demo — Scenario 2 (Brain → Music Sequence) — `demo-scenario2-music-sequence.mp4` (~0:45)**  
**Jordan (over video)**
...
That includes I2C scan on the Brain, config diff, JSON serialization, TCP send, parsing on the Flutter side, and rendering the updated UI. Our target was a mean of 2 seconds or less; the 10‑run table here shows a mean of roughly 50 milliseconds, so again we’re well below the target.  
...
“Spec 3 focuses on electrical integrity of the I2C bus. We measure the 10‑to‑90% rise time on SCL at the Brain block I2C header while the system is scanning.  

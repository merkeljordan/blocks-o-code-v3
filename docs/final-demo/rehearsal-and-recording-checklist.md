...
- **Spec 1 & Spec 2 videos** are re-used from the midterm demo (no re-recording needed).
- **Spec 3** measurement method for I2C rise time verified (scope or equivalent) with all **15 blocks connected** and comfortable capturing 10 runs + 3 live in recording.
...
- **Part 3**: Intro the three specs; for each spec: show summary table, then call out each result. State whether target is met. Play reused recordings for Spec 1 and Spec 2. Play new recordings for Spec 3. Stay within 4–6 min.

...
#define REG_WHOAMI                                                             \
  0x00 // 1 byte: block_type_t (children may mirror type; Brain uses fixed I2C
       // address)
...
// parsers can deterministically distinguish register reads from command
// packets.
// ============================================================================
...
typedef enum {
  ...
  CMD_SET_I2C_ADDRESS =
      0x8A, // Child: rebind slave address (Brain firmware does not send this)
...
// firmware_blocks/block_templates/*. Brain infers block_type_t from this
// address (no readdressing). Duplicate SKUs (0x0C..0x12) share one logical type
// per family. UID uses block_compute_device_uid().
//
// 0x08..0x0B  control-flow (unique type per image)
// 0x0C..0x12  duplicate-type product SKUs (NOTE / MUSIC_SEQ / LED_FLASH
// variants) 0x13..0x16  remaining unique types
...
/** Infer canonical child block_type_t from fixed boot I2C address (see
 * BLOCK_BOOT_I2C_ADDR_*). */
static inline block_type_t
block_infer_type_from_child_i2c_address(uint8_t address) {
  switch (address) {
...
// - IF/LOOP are evaluated at each program-counter context (not pre-frozen
// globally).

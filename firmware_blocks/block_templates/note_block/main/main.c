...
#define BLOCK_I2C_ADDRESS BLOCK_BOOT_I2C_ADDR_NOTE_BLOCK
...
// Spinlock protecting pending-event shared state accessed from multiple
// tasks/cores.
...
// ============================================================================
typedef enum { PLAYBACK_SINGLE, PLAYBACK_SEQUENCE } playback_type_t;
...
// Block-originated event payload (Brain reads via CMD_GET_DATA when
// STATUS_DATA_READY is set).
...
static bool config_is_valid(void) { return s_config_valid; }
...
static void show_note_color(uint8_t note_id) {
  ...
  matrix_fill(k_note_colors[idx].r, k_note_colors[idx].g, k_note_colors[idx].b);
  matrix_show();
}
...
static void play_note(uint8_t note_id, bool restore_idle) {
  ...
  const uint32_t count =
      (uint32_t)(sizeof(k_note_freqs_hz) / sizeof(k_note_freqs_hz[0]));
...
void note_block_preview_note(uint8_t note_id) { play_note(note_id, true); }
...
// ============================================================================
void command_handle(i2c_command_t cmd, const uint8_t *rx, size_t rx_len,
                    uint8_t *tx, size_t *tx_len) {
  if (tx_len) {
    *tx_len = 0;
  }

  (void)status_strip_handle_matrix_command(TAG, &kStatusStripConfig, cmd, rx,
                                           rx_len);
  if (status_strip_handle_runtime_broadcast(TAG, &kStatusStripConfig,
                                            BLOCK_TYPE_NOTE, cmd, rx, rx_len)) {
    return;
  }
  ...
  case CMD_PLAY_NOTE:
    if (rx && rx_len >= 1) {
      if (s_playback_queue != NULL) {
        playback_cmd_t cmd_play = {.type = PLAYBACK_SINGLE, .note_id = rx[0]};
...
void app_main(void) {
  startup_power_guard();
  ESP_LOGI(TAG, "========================================");
  ESP_LOGI(TAG, "    NOTE BLOCK BOOT (%s @ 0x%02X)", BLOCK_TYPE_NOTE_STR,
           BLOCK_I2C_ADDRESS);
  ESP_LOGI(TAG, "========================================");
  ...
  xTaskCreatePinnedToCore(note_playback_task, "note_playback", 4096, NULL, 4,
                          NULL, 1);

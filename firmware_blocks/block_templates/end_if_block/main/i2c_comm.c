...
extern void command_handle(i2c_command_t cmd,
                           ...
                           uint8_t *tx,
                           size_t *tx_len);
extern uint8_t end_if_block_get_status_flags(void);

static const char *TAG = "END_IF_BLOCK";

// TODO: Change per block
#define MY_ADDRESS      BLOCK_BOOT_I2C_ADDR_END_IF_BLOCK
...
static void init_registers(void) {
    if (s_runtime_address == 0u) {
        s_runtime_address = BLOCK_BOOT_I2C_ADDR_END_IF_BLOCK;
    }
...
{
    registers[REG_STATUS] = end_if_block_get_status_flags();
}

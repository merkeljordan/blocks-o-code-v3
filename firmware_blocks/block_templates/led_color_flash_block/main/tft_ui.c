...
/* TFT + touch wiring and runtime config (same stack as brain_block v9 setup). */
...
static void run_digit_sequence(uint8_t digit)
{
    ...
    /* 1-9 map to 1-3 flashes. */
    const uint8_t flashes = ((digit - 1U) % 3U) + 1U;
...
static void submit_btn_event_cb(lv_event_t *e)
{
    ...
    if (s_status_label != NULL) {
        lv_label_set_text_fmt(s_status_label, "Submitted! Running digit %u", (unsigned)digit);
    }
}

static lv_obj_t *create_key(lv_obj_t *parent, uint8_t digit, lv_coord_t x, lv_coord_t y)
{
    ...
    lv_obj_set_style_bg_color(btn, lv_color_darken(color, LV_OPA_20), LV_STATE_PRESSED);
    lv_obj_set_style_text_color(btn, lv_color_hex(0x101010), 0);
    lv_obj_add_event_cb(btn, numpad_btn_event_cb, LV_EVENT_CLICKED, (void *)(uintptr_t)digit);
...
static lv_obj_t *create_numpad_screen(void)
{
    ...
    lv_obj_set_style_bg_color(scr, lv_color_hex(0x1F2A44), 0);
    ...
    const lv_coord_t x0 = 12;
    const lv_coord_t y0 = 62;
    const lv_coord_t dx = 76;
    const lv_coord_t dy = 52;
    ...
    lv_obj_set_style_radius(submit_btn, 12, 0);
...
static lv_obj_t *create_intro_screen(void)
{
    ...
    lv_obj_set_style_radius(scr, 0, 0);
    lv_obj_set_style_pad_all(scr, 0, 0);
    lv_obj_set_style_bg_color(scr, lv_color_hex(0x7AD7F0), 0);
    lv_obj_set_style_bg_grad_color(scr, lv_color_hex(0xEAFBFF), 0);
    lv_obj_set_style_bg_grad_dir(scr, LV_GRAD_DIR_VER, 0);

    lv_obj_t *title = lv_label_create(scr);
    lv_label_set_text(title, "Welcome!");
    lv_obj_set_style_text_color(title, lv_color_hex(0x1A2340), 0);
    ...
    lv_obj_set_style_text_color(subtitle, lv_color_hex(0x1A2340), 0);
    ...
    lv_obj_align(start_btn, LV_ALIGN_BOTTOM_MID, 0, -24);
    lv_obj_set_style_radius(start_btn, 14, 0);
    lv_obj_set_style_bg_color(start_btn, lv_color_hex(0xFFD166), 0);
    lv_obj_set_style_bg_color(start_btn, lv_color_hex(0xF4B942), LV_STATE_PRESSED);
...
static void open_numpad_screen(void)
{
    ...
    lv_screen_load_anim(s_numpad_screen, LV_SCR_LOAD_ANIM_MOVE_LEFT, 200, 0, false);
}
...
void tft_ui_start(void)
{
    ...
    BaseType_t ok = xTaskCreatePinnedToCore(lvgl_task, "led_ui_v9", TFT_TASK_STACK_SIZE, NULL,

...
static void command_action_task(void *arg) {
    ...
    while (1) {
        ...
        if (action.type == ACTION_PREVIEW && s_execute_inflight) {
            ESP_LOGI(TAG, "Skipping preview while execute inflight");
            led_matrix_set_status_mirror(false);
        ...
        if (track_busy) {
            ...
            if (!s_execute_inflight) {
                set_current_status(compose_nonbusy_status(true, s_pending_event_valid));
                ESP_LOGI(TAG, "Worker finished preview action=%u pending_event=%d",
                         (unsigned)action.type, (int)s_pending_event_valid);
            } else {
                ESP_LOGI(TAG, "Preview completed while execute inflight; preserving BUSY");
            }
        }
        led_matrix_set_status_mirror(false);
        s_effect_active = false;

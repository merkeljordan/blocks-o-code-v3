...
// ============================================================================
// STUB FUNCTIONS FOR SPI-ONLY MODE
// ============================================================================
// When CONFIG_SPI_INTERFACE is set, these I2C functions are stubbed out
// The linker needs them to exist, but they should never be called at runtime
#ifdef CONFIG_SPI_INTERFACE

void i2c_init(SSD1306_t * dev, int width, int height) {
    // Should never be called in SPI mode
    ESP_LOGE("SSD1306", "ERROR: i2c_init called in SPI-only mode!");
}

void i2c_display_image(SSD1306_t * dev, int page, int seg, const uint8_t * images, int width) {
    // Should never be called in SPI mode
    ESP_LOGE("SSD1306", "ERROR: i2c_display_image called in SPI-only mode!");
}

void i2c_contrast(SSD1306_t * dev, int contrast) {
    // Should never be called in SPI mode
    ESP_LOGE("SSD1306", "ERROR: i2c_contrast called in SPI-only mode!");
}

void i2c_hardware_scroll(SSD1306_t * dev, ssd1306_scroll_type_t scroll) {
    // Should never be called in SPI mode
    ESP_LOGE("SSD1306", "ERROR: i2c_hardware_scroll called in SPI-only mode!");
}

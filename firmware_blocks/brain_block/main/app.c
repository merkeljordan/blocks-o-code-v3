...
Bi-directional over LAN: ESP connects to a desktop server (always-listening),
...
- When Wi‑Fi is connected, starts a TCP client task that connects to
...
#define WIFI_SSID "Chung Yong's Galaxy S21+ 5G"      // <-- Set your Wi‑Fi SSID here
#define WIFI_PASS "kisd8101" // <-- Set your Wi‑Fi password here

/* Desktop server IP and port to connect to (set to your desktop listening
 * server) */
#define SERVER_IP                                                              \
  "10.55.198.154" // <-- Set your server's IP address here (ipconfig)
...
#define BLOCK_CONFIG_SCAN_INTERVAL_MS_IDLE 50
#define BLOCK_CONFIG_SCAN_INTERVAL_MS_ACTIVE 450
#define BLOCK_CONFIG_SCAN_TASK_STACK_SIZE 8192
#define TCP_CLIENT_TASK_STACK_SIZE 8192
#define JSON_HEAP_LOG_INTERVAL_MS 5000ULL
...
static char s_block_config_json[BLOCK_CONFIG_JSON_BUFFER_BYTES];
/* Keep scan JSON buffer out of task stack to avoid overflow in block_cfg_scan.
 */
static char s_block_config_scan_json_buffer[BLOCK_CONFIG_JSON_BUFFER_BYTES];
static size_t s_block_config_json_len = 0;
static bool s_block_config_json_valid = false;

static void log_json_heap_if_due(const char *source) {
  static uint64_t s_last_heap_log_ms = 0;
  uint64_t now_ms = (uint64_t)(esp_timer_get_time() / 1000);

  if ((now_ms - s_last_heap_log_ms) < JSON_HEAP_LOG_INTERVAL_MS) {
    return;
  }

  s_last_heap_log_ms = now_ms;
  ESP_LOGI(TAG, "[%s] free_heap=%lu", source,
           (unsigned long)esp_get_free_heap_size());
}

static bool copy_latest_block_config_json(char *out, size_t out_size,
                                          size_t *out_len) {
  ...
  bool ok = false;
  if (s_block_config_json_valid && s_block_config_json_len > 0 &&
      s_block_config_json_len < out_size) {
...
static const char *
runtime_state_to_json_string(brain_runtime_broadcast_state_t state) {
  switch (state) {
...
static bool build_runtime_update_json(char *out, size_t out_size,
                                      size_t *out_len) {
  ...
  const brain_runtime_snapshot_t *runtime =
      brain_event_handler_get_runtime_snapshot();
  if (runtime == NULL) {
    return false;
  }
...
static bool runtime_snapshot_equals(const brain_runtime_snapshot_t *a,
  ...
  return a->state == b->state && a->pc == b->pc &&
         a->step_type == b->step_type && a->updated_at_ms == b->updated_at_ms;
}
...
static void block_config_scan_task(void *pvParameters) {
  ...
  while (1) {
    ...
    log_json_heap_if_due("block_cfg_scan");
    ...
    if (config_changed || !s_block_config_json_valid) {
      esp_err_t jerr = block_config_manager_get_json(
          s_block_config_scan_json_buffer,
          sizeof(s_block_config_scan_json_buffer));
      if (jerr == ESP_OK) {
        ...
        if (s_block_config_json_mutex != NULL &&
            xSemaphoreTake(s_block_config_json_mutex, pdMS_TO_TICKS(200)) ==
                pdTRUE) {
          if (json_len < sizeof(s_block_config_json)) {
            memcpy(s_block_config_json, s_block_config_scan_json_buffer,
                   json_len);
        ...
        if (s_block_config_event_group != NULL) {
          xEventGroupSetBits(s_block_config_event_group,
                             BLOCK_CONFIG_CHANGED_BIT);
        }
      } else {
        ESP_LOGE(TAG,
                 "block_config_manager_get_json failed (%s); keeping previous "
                 "JSON (companion may be stale)",
                 esp_err_to_name(jerr));
      }
    }

    uint32_t scan_ms = brain_executor_prefers_i2c_yield()
                           ? BLOCK_CONFIG_SCAN_INTERVAL_MS_ACTIVE
                           : BLOCK_CONFIG_SCAN_INTERVAL_MS_IDLE;
...
static void tcp_client_task(void *pvParameters) {
  ...
  char *json_buffer = malloc(BLOCK_CONFIG_JSON_BUFFER_BYTES);
  brain_runtime_snapshot_t last_sent_runtime = {0};
  bool last_sent_runtime_valid = false;

  if (json_buffer == NULL) {
    ESP_LOGE(TAG, "Failed to allocate JSON buffer for tcp_client_task");
    vTaskDelete(NULL);
    return;
  }
  ...
  while (1) {
    ...
    ESP_LOGI(TAG, "Attempting to connect to server %s:%d", SERVER_IP,
             SERVER_PORT);
    ...
    /* Short recv timeout so we wake often and push config updates quickly (~50
     * ms). */
    struct timeval rcv_timeout;
    rcv_timeout.tv_sec = 0;
    rcv_timeout.tv_usec = 50000; /* 50 ms */
    setsockopt(sock, SOL_SOCKET, SO_RCVTIMEO, &rcv_timeout,
               sizeof(rcv_timeout));
    struct timeval snd_timeout;
    snd_timeout.tv_sec = 5;
    snd_timeout.tv_usec = 0;
    setsockopt(sock, SOL_SOCKET, SO_SNDTIMEO, &snd_timeout,
               sizeof(snd_timeout));
    ...
    size_t json_len = 0;
    if (!copy_latest_block_config_json(
            json_buffer, BLOCK_CONFIG_JSON_BUFFER_BYTES, &json_len)) {
      // Wait briefly for the scan task to populate initial JSON.
      if (s_block_config_event_group != NULL) {
        xEventGroupWaitBits(s_block_config_event_group,
                            BLOCK_CONFIG_CHANGED_BIT,
                            pdTRUE, // clear on exit
                            pdFALSE, pdMS_TO_TICKS(1500));
      }
      (void)copy_latest_block_config_json(
          json_buffer, BLOCK_CONFIG_JSON_BUFFER_BYTES, &json_len);
    }
    ...
    const brain_runtime_snapshot_t *initial_runtime =
        brain_event_handler_get_runtime_snapshot();
    if (initial_runtime != NULL) {
      char runtime_json[256];
      size_t runtime_len = 0;
      if (build_runtime_update_json(runtime_json, sizeof(runtime_json),
                                    &runtime_len) &&
          runtime_len > 0) {
        int written = send(sock, runtime_json, runtime_len, 0);
        if (written < 0) {
          ESP_LOGE(TAG, "Error sending initial runtime update: errno %d",
                   errno);
        } else {
    ...
    while (1) {
      ...
      if (s_block_config_event_group != NULL) {
        ...
        if (cfg_bits & BLOCK_CONFIG_CHANGED_BIT) {
          ...
          if (copy_latest_block_config_json(
                  json_buffer, BLOCK_CONFIG_JSON_BUFFER_BYTES, &updated_len) &&
              updated_len > 0) {
      ...
      const brain_runtime_snapshot_t *runtime =
          brain_event_handler_get_runtime_snapshot();
      if (runtime != NULL &&
          (!last_sent_runtime_valid ||
           !runtime_snapshot_equals(&last_sent_runtime, runtime))) {
        char runtime_json[256];
        size_t runtime_len = 0;
        if (build_runtime_update_json(runtime_json, sizeof(runtime_json),
                                      &runtime_len) &&
      ...
      if (len > 0) {
          ...
          if (type_item != NULL && cJSON_IsString(type_item)) {
            ...
            if (strcmp(type, "heartbeat") == 0) {
              ...
              if (ack_json != NULL) {
                ...
                cJSON_AddNumberToObject(ack_json, "timestamp",
                                        (double)(esp_timer_get_time() / 1000));
                ack_string = cJSON_PrintUnformatted(ack_json);
              }
              if (ack_string != NULL) {
              ...
              cJSON *error_count_item =
                  cJSON_GetObjectItem(json, "error_count");
              cJSON *timestamp_item = cJSON_GetObjectItem(json, "timestamp");

              bool is_valid =
                  cJSON_IsBool(is_valid_item) && cJSON_IsTrue(is_valid_item);
              uint32_t error_count =
                  cJSON_IsNumber(error_count_item)
              ...
              brain_event_handler_set_config_validation(is_valid, error_count,
                                                        timestamp_ms);
              s_validation_requested_by_start = false;
              ESP_LOGI(TAG,
                       "Applied config_validation: valid=%s errors=%lu ts=%llu",
                       is_valid ? "true" : "false", (unsigned long)error_count,
        ...
        while (line) {
          ...
          if (strcasecmp(line, "START") == 0) {
            ...
            const brain_validation_state_t *validation =
...
bool brain_companion_is_connected(void) { return s_companion_connected; }
...
void start_network_client(void) {
  ...
  xTaskCreatePinnedToCore(block_config_scan_task, "block_cfg_scan",
                          BLOCK_CONFIG_SCAN_TASK_STACK_SIZE, NULL, 4, NULL, 0);

  wifi_init_sta();

  /* Start TCP client task on Core 0 to keep Core 1 available for GUI. */
  xTaskCreatePinnedToCore(tcp_client_task, "tcp_client_task",
                          TCP_CLIENT_TASK_STACK_SIZE, NULL, 5, NULL, 0);
}
